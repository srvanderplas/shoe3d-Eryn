...
centering<-Arothron::bary.mesh(sony1) #this is added now for the shoe_grab for the longitudinal shoes
sony1<-translate3d(sony1, -centering[1], -centering[2], -centering[3])
stl_file_path2 <- stl_new[str_detect(stl_new, "SonyL")][2]
sony2 <- Rvcg::vcgImport(stl_file_path2, clean = T)
centering<-Arothron::bary.mesh(sony2)
sony2<-translate3d(sony2, -centering[1], -centering[2], -centering[3])
```
...
```{r}
<<<<<<< HEAD
#colMeans1 <- colMeans(sony1shoe)
#sony1c <- translate3d(sony1, -colMeans1[1], -colMeans1[2], -colMeans1[3])
rgl::plot3d(sony1, aspect="iso", col="blue", add=TRUE)
rgl::plot3d(sony11, aspect="iso", col="green", add=TRUE)
=======
colMeans1 <- colMeans(sony1shoe)
bary1 <- barycenter(sony1) %>% colMeans()
sony1c <- translate3d(sony1, -bary1[1], -bary1[2], -bary1[3])
rgl::plot3d(sony1c, aspect="iso", col="blue", add=TRUE)
>>>>>>> bff0c172e7a989d8cfaca6f32a4abfd90ed03a11
axes3d()

#colMeans2 <- colMeans(sony2shoe)
#sony2c <- translate3d(sony2, -colMeans2[1], -colMeans2[2], -colMeans2[3])
...
a<-fastKmeans(sony1shoe, 3, iter.max = 10, project = TRUE, threads = 0)
spheres3d(a$centers)
```
...
```{r}
rotationMatrix(matrix = sony1pca$rotation)
```
...
Trying to switch them to homogeneous coordinates 



```{r}
hommat<-asHomogeneous(sony1lowpca$rotation)
hommat
```

```{r}
rgl::plot3d(sony1low, aspect="iso", col="blue", add=TRUE)
par3d(userProjection=hommat)
```
